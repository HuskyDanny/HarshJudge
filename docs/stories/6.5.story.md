...
- [ ] Task 1: Update SKILL.md with step-based agent pattern (AC: 1, 5)
  - [ ] Open `skills/harshjudge/SKILL.md`
  - [ ] Add "Step-Based Execution" section explaining agent spawning
  - [ ] Document per-step agent isolation pattern
  - [ ] Include agent prompt template for step execution
  - [ ] Remove reference to deprecated `iterations.md`

- [ ] Task 2: Rewrite run.md for step-based workflow (AC: 1, 2, 3, 4)
  - [ ] Open `skills/harshjudge/references/run.md`
  - [ ] Update workflow to: read meta.yaml → loop through steps → spawn agent per step
  - [ ] Define step agent prompt template with: step content, PRD summary, previous result
  - [ ] Define step agent return format: status, evidencePaths, error
  - [ ] Include progress tracking with result.json updates
  - [ ] Embed Playwright tools reference (from Story 6.6)

- [ ] Task 3: Create step execution agent prompt template (AC: 2, 3)
  - [ ] Design prompt template with clear boundaries:
    ```
    ...
    ```
  - [ ] Include Playwright tool usage guidance
  - [ ] Include evidence naming conventions

- [ ] Task 4: Update startRun to return step list (AC: 4)
  - [ ] Verify `startRun` returns `steps` array (from meta.yaml)
  - [ ] Ensure step list includes id, title, file for each step
  - [ ] Main agent uses this list to orchestrate execution

- [ ] Task 5: Define main agent orchestration flow (AC: 4)
  - [ ] Document the orchestration loop in run.md:
    ```
...
- [ ] Task 6: Update iterate.md for step-based scenarios (AC: 1)
  - [ ] Open `skills/harshjudge/references/iterate.md`
  - [ ] Update to reference individual step files
  - [ ] Update to use per-step evidence directories
  - [ ] Remove references to monolithic scenario.md

- [ ] Task 7: Write integration test scenario (AC: 1-5)
  - [ ] Create test scenario in `examples/` or document manual test
  - [ ] Verify token isolation works (agent context is isolated)
  - [ ] Verify progress tracking in result.json
  - [ ] Verify step failure stops execution
