  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-gray-900">
      ...
      <div className="flex-1 overflow-auto" role="tabpanel">
        ...
        {activeTab === 'logs' && <LogsTabContent logs={evidence.logs} />}
        {activeTab === 'db' && (
...
const LogsTabContent: FC<{ logs: StepEvidence[] }> = ({ logs }) => {
  ...
  return (
    <div className="p-4 space-y-3">
      {logs.map((log, index) => (
        <EvidenceContentItem key={`${log.path}-${index}`} evidence={log} />
      ))}
    </div>
  );
};
  ...
  return (
    <div className="border border-gray-700 rounded-lg overflow-hidden">
      ...
      <div className="bg-gray-900">
        ...
        {content && (
          <LogSection title="" content={content} defaultExpanded={true} />
        )}
      </div>
    </div>
  );
