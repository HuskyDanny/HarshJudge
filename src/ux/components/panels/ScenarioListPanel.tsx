...
import { type ScenarioSummary } from '../../../types/index.js';
  ...
  if (error) {
    return (
      <div className="flex flex-col h-full">
        ...
        <EmptyState title="Error loading scenarios" description={error} />
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full">
      ...
      <header className="flex items-center justify-between p-3 border-b border-gray-700">
        <h2 className="text-sm font-medium text-gray-400">
          Scenarios (
          {showStarredOnly ? sortedScenarios.length : scenarios.length})
        </h2>
        {starredCount > 0 && (
          <button
              ...
              <div className="flex items-center gap-2 mb-1">
                {scenario.stepCount > 0 && (
                  <span className="text-xs text-gray-500 bg-gray-800 px-1.5 py-0.5 rounded">
                    {scenario.stepCount}{' '}
                    {scenario.stepCount === 1 ? 'step' : 'steps'}
                  </span>
                )}
